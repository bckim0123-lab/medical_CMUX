...
// Function-calling Data Agent — OpenAI 우선, Gemini 차순, 직접호출 최종.
...
export const dataAgent: Agent = {
  ...
  async *run(state: OrchestrationState): AsyncGenerator<AgentEvent, Partial<OrchestrationState>, void> {
    ...
    // 우선순위: OpenAI → Gemini → direct
    if (hasOpenAIKey()) {
      try {
        return yield* runOpenAI(state);
      } catch (err) {
        yield { type: 'error', agent: 'data', message: `OpenAI 실패: ${msg(err)} — 다음 provider 시도` };
      }
    }
    if (hasGeminiKey()) {
      try {
        return yield* runGemini(state);
      } catch (err) {
        yield { type: 'error', agent: 'data', message: `Gemini 실패: ${msg(err)} — 직접 호출` };
      }
    }
    yield { type: 'log', agent: 'data', message: 'LLM 키 미설정/실패 → 직접 호출 모드' };
...
async function* runGemini(state: OrchestrationState): AsyncGenerator<AgentEvent, Partial<OrchestrationState>, void> {
  const ai = getGemini();
  const hospitalsRef = { current: [] as Hospital[] };
  const populationRef = { current: [] as PopulationDong[] };
  ...
  while (safety++ < 4) {
    ...
    for (const call of calls) {
      ...
      yield { type: 'tool', agent: 'data', tool: call.name, args };
      const result = await execTool(call.name, hospitalsRef, populationRef);
  ...
  if (summary?.trim()) yield { type: 'log', agent: 'data', message: summary.trim() };
