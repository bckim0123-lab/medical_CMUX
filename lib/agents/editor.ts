...
// Tries Gemini first, then OpenAI; falls back to a deterministic template
// render so the demo never hard-fails.
export const editorAgent: Agent = {
  ...
  async *run(state: OrchestrationState): AsyncGenerator<AgentEvent, Partial<OrchestrationState>, void> {
    ...
    if (!hasAnyLlmKey()) {
      yield { type: 'log', agent: 'editor', message: 'LLM 키 미설정 → 템플릿 렌더링' };
      return { report: templateReport(state) };
    }

    yield { type: 'tool', agent: 'editor', tool: 'generate_markdown_report' };
    ...
    try {
      const { text, provider } = await generateText({
        ...
        prompt:
          `다음 분석 결과를 바탕으로 [최종 정책 제언 리포트]를 작성하세요.\n\n` +
      ...
      yield { type: 'log', agent: 'editor', message: `${provider} 응답 ${text.length}자 수신` };
      return { report: text };
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      yield { type: 'error', agent: 'editor', message: `LLM 실패 — 템플릿으로 fallback: ${msg}` };
      return { report: templateReport(state) };
    }
  },
};
